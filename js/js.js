		...
		window.onload = function(){
			...
			function tick(){
				...
				for (var i = aImg.length - 1; i >= 0; i--) {
					move(aImg[i],{top:-35*str.charAt(i)},{duration:300});
				}
			}
			tick();
			...
			(function(){
				...
				oScroll.onclick=function(){
					addClass(this,'on');
					bOk=true;
					oScroll.style.transtion = '0.5s all ease-in';
	...
	(function(){
		...
		document.onmousemove=function(ev){
			...
			for (var i = 0; i < aImg.length; i++) {
				var a=aImg[i].offsetTop+oDock.offsetTop+aImg[i].offsetHeight/2-oEvent.clientY;
				var b=aImg[i].offsetLeft+aImg[i].offsetWidth/2-oEvent.clientX;
